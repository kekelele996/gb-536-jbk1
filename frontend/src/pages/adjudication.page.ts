      ...
      <section class="evidence-panel">
        @if (cases.selected(); as item) {
          ...
          @if (item.case_state === 'pending_recompute') {
            <aside class="retired-banner"><lucide-icon name="rotate-ccw" [size]="16" /><span><strong>Pending recomputation</strong>An input result was replaced. This frozen evidence is retained for inspection but left the active queue; recompute with the new revision to continue.</span></aside>
          }
          @if (item.case_state === 'superseded') {
            <aside class="retired-banner history"><lucide-icon name="archive" [size]="16" /><span><strong>Superseded history</strong>Replaced by case #{{ item.superseded_by_case_id }}. It is read-only and no longer drives the current decision.</span></aside>
          }
          @if (item.case_state === 'accepted') {
            <aside class="retired-banner history"><lucide-icon name="lock-keyhole" [size]="16" /><span><strong>Accepted history</strong>The accepted decision is preserved read-only even if an input result is later replaced.</span></aside>
          }
