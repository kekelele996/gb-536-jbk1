    ...
    <section class="case-layout">
      ...
      <section class="evidence-panel">
        @if (cases.selected(); as item) {
          ...
          @if (item.case_state === 'pending_recompute') {
            <aside class="staleness pending"><lucide-icon name="rotate-ccw" [size]="16" /><span><strong>Pending recompute</strong>One or more compared result sets (#{{ item.superseded_set_ids.join(', #') }}) were replaced after this comparison. The case no longer enters assignment or decisions; run a fresh comparison with the replacement revisions.</span></aside>
          } @else if (!item.evidence_current) {
            <aside class="staleness accepted"><lucide-icon name="archive" [size]="16" /><span><strong>Accepted history · superseded evidence</strong>This decision is retained for viewing only. Its frozen evidence references replaced result sets (#{{ item.superseded_set_ids.join(', #') }}) and can no longer act as the current version.</span></aside>
          }
          ...
          @if (canAdjudicate() && item.evidence_current) {
